...
public final class Quaternion implements Serializable {
	...
	/**
     * Builds a rotation quaternion from a vector axis to be rotated about and the angle of rotation in degree
	 * @param v      Components of the vector part of the quaternion.
	 * @param degree Degree to rotate about vector v.
     * @throws DimensionMismatchException if the array length is not 3.
     */
 	public Quaternion(final double[] v, final double degree) {
 		if (v.length != 3) {
			throw new DimensionMismatchException(v.length, 3);
		}
 		double pi = Math.PI;
 		double angle = pi/180 * degree;
 		this.q0 = Math.cos(angle / 2);
	...
	/**
	 * Builds a pure quaternion from a vector (assuming that the scalar part is
	 * zero).
	 *
	...
	public static Quaternion add(final Quaternion q1, final Quaternion q2) {
		return new Quaternion(q1.getQ0() + q2.getQ0(), q1.getQ1() + q2.getQ1(), q1.getQ2() + q2.getQ2(),
				q1.getQ3() + q2.getQ3());
	}
	...
	/**
	 * Computes the normalized quaternion (the versor of the instance). The norm of
	 ...
	 * @param eps Tolerance.
	 * @return {@code true} if the each of the components are equal within the
	...
	public boolean equals(final Quaternion q, final double eps) {
		return Precision.equals(q0, q.getQ0(), eps) && Precision.equals(q1, q.getQ1(), eps)
				&& Precision.equals(q2, q.getQ2(), eps) && Precision.equals(q3, q.getQ3(), eps);
	}
	...
	/**
	 * Returns the inverse of this instance. The norm of the quaternion must not be
	 * zero.
	 *
	 * @return the inverse.
	...
	public static void main(String[] args) {
		...
		Quaternion pureq = new Quaternion(v1); // pure quaternion for vector v1		
		Quaternion qresult = new Quaternion(0,0,0,0);
		...
		for (int i = 0; i < CAP; i++)
		{
			Quaternion q = new Quaternion(vx, 90);
			Quaternion qconj = q.getConjugate();
			qresult = q.multiply(pureq).multiply(qconj);
			//System.out.println("vector v rotate 90 degree about the x axis is: " + qresult);
			//pureq = qresult.normalize();
			pureq = new Quaternion(0, qresult.getVectorPart());
		}
		System.out.println("the result quaternion is: " + qresult);
		long end = System.currentTimeMillis();
		long diff = end - start;
		System.out.println("rotation using quaternion takes: "+ diff + " milliseconds");
		
		long free = rt.freeMemory();
        long memoryused = (prevFree - free)/1000000;
        System.out.println("rotation using quaternion takes: "+ memoryused + " Megabytes");
	}
}
